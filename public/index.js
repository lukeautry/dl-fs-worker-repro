...
For more info, visit https://reactjs.org/link/mock-scheduler`);
  ...
  var DB_NAME = "dl-fs-repro-db";
  var HANDLE_OBJECT_STORE = "handles";
  ...
  // src/client/idb-service.ts
  var IDBService = class {
    getDb() {
    ...
    save(fileInfo) {
      return __async(this, null, function* () {
        ...
        yield tx.store.put(fileInfo);
      });
    }
    getFileInfoFromIDB(path) {
      return __async(this, null, function* () {
        const db = yield this.getDb();
        const tx = db.transaction([HANDLE_OBJECT_STORE], "readonly");
        return yield tx.store.get(path);
      });
    }
  };
  ...
  // src/client/app.tsx
  var App = () => {
    const onClickCrash = () => __async(void 0, null, function* () {
      const directory = yield showDirectoryPicker();
      yield directory.requestPermission({ mode: "readwrite" });
      const handle = yield directory.getFileHandle(FILE_NAME, {
        create: true
      });
      yield new IDBService().save({
        path: FILE_NAME,
        handle
      });
